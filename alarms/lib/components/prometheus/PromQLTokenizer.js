...
import { AGGREGATION_OPERATORS, AGGR_CLAUSE_TYPES, BINARY_ARITHMETIC_OPS, BINARY_COMPARATORS, BINARY_SET_OPS, GROUP_CLAUSE_TYPES, LABEL_OPERATORS, MATCH_CLAUSE_TYPES, SyntaxError } from './PromQLTypes';
const lexerRules = {
  WS: /[ \t]+/,
  lBrace: '{',
  rBrace: '}',
  ...
  range: {
    match: /[0-9]+[smhdwy]/,
    value: s => new Range(parseInt(s.substring(0, s.length - 1), 10), s.substring(s.length - 1))
  },
  scalar: [{
    // binary integers
    match: /0b[01]+/,
...
function unescapeCharAt(s, i, quote) {
  ...
  if (currentChar in simpleUnescaper) {
    return {
      char: simpleUnescaper[currentChar],
      newIndex: currentIndex + 1
    };
  }
...
function unescapeHexAt(s, i, width) {
  if (i >= s.length - width) {
    throw new SyntaxError(unterminatedEscape);
  }

  const hex = s.substring(i, i + width); // this will happily parse e.g. 'fz' into 15
...
function unescapeOctAt(s, i) {
  if (i >= s.length - 3) {
    throw new SyntaxError(unterminatedEscape);
  }

  const oct = s.substring(i, i + 3); // this will happily parse e.g. '2T' or '29' into 2
  // this bug is left here intentionally, to balance correctness vs simplicty
...
export const lexer = Moo.compile(lexerRules); // Ignore whitespace and comment tokens
